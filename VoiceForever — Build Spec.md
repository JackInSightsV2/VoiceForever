...
**2. Approval page.** Approve every Candidate you'd be happy to hear as an NPC of that race. Each approved Candidate becomes one of the Archetype's **Base Voices**, a different person, up to 8 per Archetype (ADR-0006). **Approve** toggles: it adds a Base Voice without demoting the others, and **Unapprove** withdraws one. **Reject** still rejects. Per Archetype you can also **Regenerate with a note** (e.g. "deeper, less theatrical"): the note is appended to the Archetype's description and a new batch of Candidates is rendered, and Base Voices already approved stay. The actions are queued, and the next `vo prepare` applies them. Every Archetype needs at least one Base Voice. Per Lexicon entry you can accept or correct the spelling. Approval writes a locked `approved_voices.json` and `lexicon.json`, and `vo run` refuses to start without them. Names outside the top 300 are drafted automatically and checked by ASR.
...
| Approval | Every race and gender with its Candidates and test lines, how many are approved as Base Voices; the Narrator; the top Lexicon entries with samples | Approve / unapprove (toggle, up to 8 Base Voices per Archetype), reject, regenerate with note; accept or correct spelling |
...
Every NPC with dialogue gets a unique NPC Voice, derived automatically from one Base Voice of the approved Archetype for its race and gender. Each voice must stay close enough to its Base Voice to sound like the approved race, and far enough from its Neighbours to sound distinct.
...
**1. Archetypes.** Every race and gender in the data, plus each Creature Family, has a description (race style guide). `vo prepare` renders several VoxCPM2 voice-design candidates of an anchor line per Archetype, and on the Approval page you approve every one with the right character, up to 8. Each approved one is a **Base Voice**: a different person of that race (ADR-0006). A race that needs it (e.g. orc male; later perhaps Undead or the Great Beasts family) also gets an effect chain, applied once to each candidate anchor: the processed clip is the anchor, and every line is a plain continuation of it, so the character carries into every line and the voice stays one person (bake-off round 5: the chain applied per line lost the character). Orc male's candidates are seeded from the bake-off anchors (`vo prepare --import-bakeoff orc_m`) rather than freshly designed.

**2. Base Voice.** `vo voices` first assigns each NPC one of its Archetype's Base Voices, so 357 human males are 8 people with variations rather than 1 person with 357. The assignment is deterministic and balanced. It is also Neighbour-aware: in NPC id order, each NPC takes the Base Voice least used among its same-Archetype Neighbours assigned so far, and ties go to the least-used Base Voice overall. The pick is stored per NPC (`voice_builds.anchor`) and kept while the Archetype's Base Voices stay the same. Approving or withdrawing a Base Voice re-assigns that Archetype's NPCs, and those whose Base Voice changed are rebuilt. `vo voices` reports the spread per Archetype: NPCs per Base Voice, and how many same-Archetype Neighbour pairs share one. On Core Content, 8 human_m Base Voices get 44–45 NPCs each, and 103 of 1,915 Neighbour pairs share one.

**NPC anchor.** Each NPC then gets its own anchor: its Base Voice, shifted by a small seeded DSP variation (pitch ±0.8–2 semitones, formant up to ±6%, pace up to ±6%; wider for named NPCs). Every line continues from the shifted anchor, so the Base Voice's character is kept by construction (ADR-0005). The seed comes from the NPC ID, so rebuilds are deterministic.

Four candidates are made per NPC. Each must stay close to its Base Voice: speaker similarity at or above the ceiling, and pitch and HNR within a band around the Base Voice's. The one chosen is the closest to the Base Voice that still clears the Neighbour floor.
...
**5. Two-sided constraint.** Every NPC Voice must satisfy two checks. Its distance to its **Base Voice** must stay below a ceiling, so it still sounds like the voice you approved. Its distance to each **Neighbour** must stay above a single floor, so it sounds distinct. Violations are resolved by re-rolling traits and regenerating the NPC anchor, up to a cap. Anything still failing is listed in the morning report and does not block the run. It is recorded in `voice_builds` (`vo voices --leftovers`, and the Separation page). An NPC with no candidate that sounds like its Base Voice speaks with the Base Voice itself. Both thresholds live in `vo.voices.Config`. The floor (0.95, cosine) is provisional until it is calibrated by ear on about 20 pairs from the Separation page.
