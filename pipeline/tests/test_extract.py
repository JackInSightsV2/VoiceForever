...
def test_quest_parts_and_player_gender(conn, world, questie, displays):
    ...
    assert ("quest_complete", 783, 197, "m", "Ah, good, sir.") in got
    assert ("quest_complete", 783, 197, "f", "Ah, good, madam.") in got
    assert all(r[0] for r in conn.execute("SELECT tts_text IS NULL FROM lines"))
