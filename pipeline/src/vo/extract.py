...
def extract_all(conn: sqlite3.Connection, world: sqlite3.Connection, questie: Questie,
    ...
    def add(kind, quest_id, npc_id, raw):
        ...
        if not raw:
            return
        for gender, variant in text.gender_variants(raw).items():
            wanted[(kind, quest_id, npc_id, gender, variant)] = None
...
def _write(conn, npcs, spawns, wanted, zones: Zones):
    ...
    conn.execute("DELETE FROM npc_issues")
    ...
    conn.executemany("INSERT INTO lines (type, quest_id, npc_id, player_gender, raw_text) VALUES (?, ?, ?, ?, ?)",
                     [k for k in wanted if k not in existing])
